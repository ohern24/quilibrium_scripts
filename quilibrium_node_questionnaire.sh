...
last_next_difficulty=$(journalctl -u ceremonyclient -ocat -n 100 | grep difficulty | awk -F'[:,}]' '{for(i=1;i<=NF;i++){if($i~"next_difficulty_metric"){gsub(/[ "]/,"",$i); print $(i+1)}}}' | tail -n 1); diff_count=${#last_next_difficulty}; if [[ diff_count -le 1 ]]; then echo "Last Difficulty: Error getting difficulty please pull from your logs"; else echo "Last Difficulty: "$last_next_difficulty; fi;
